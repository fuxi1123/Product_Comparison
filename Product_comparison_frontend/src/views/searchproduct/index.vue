        ...
        <el-table-column label="操作" width="150">
          <template #default="scope">
            <el-button
              type="primary"
              size="small"
              @click="openAlertDialog(scope.row)"
            >
              添加提醒
            </el-button>
          </template>
    ...
    >
      <el-form :model="alertForm" label-width="120px">
        <el-form-item label="商品 ID">
          <el-input v-model="alertForm.productId" readonly></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer">
        <el-button @click="alertDialogVisible = false">取消</el-button>
        <el-button type="primary" @click="createAlert">提交</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
import { createPriceAlert } from "@/api/alert"; // 引入创建提醒的 API
...
export default {
  ...
  methods: {
    async fetchProducts() {
      ...
      this.loading = true;
      try {
        const response = await axios.get("/prod-api/product/scrape", {
    ...
    // 打开添加提醒弹窗
    openAlertDialog(product) {
      this.alertForm.productId = product.id;
    ...
    async createAlert() {
      try {
        ...
        const alertData = {
          userId: userId, // 动态设置用户 ID
          productId: this.alertForm.productId, // 商品 ID
          email: email // 用户输入的邮箱
        };

        const response = await createPriceAlert(alertData);
        if (response.code == 20000) {
          this.$message.success(response.message || "提醒创建成功！");
          this.alertDialogVisible = false;
        } else {
          this.$message.error(response.message || "创建提醒失败！");
        }
      } catch (error) {
        console.error("Error:", error);
        this.$message.error("创建提醒失败，请稍后重试！");
      }
    },
  },
};
